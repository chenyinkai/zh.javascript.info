...
大多数情况下，cookies 是 web 服务器设置的。然后它们会自动添加到相同域名下的每次请求中。
...
我们把这个留给读者当作练习，然后在本章节的结尾，你可以找到一些操作 cookies 很有效的方法。
...
cookie 可访问的 url 路径。必须是绝对路径。默认值为当前路径。
...
指定 cookie 可访问的域名。然而在实践中，存在局限性。我们不能设置任何域名。
...
……但是如果我们想要批准像 `forum.site.com` 这样的子域名访问，这是可以做到的。我们应该明确设置 `domain` 字段为根域名：`domain=site.com`：
...
默认情况下，如果一个 cookie 没有设置这两个参数中的任何一个，那么在浏览器关闭后，它就会消失。例如 "session cookies”。

为了让 cookies 在浏览器关闭后仍然存在，我们可以设置 `expires` 或 `max-age` 其中一个字段。

- **`expires=Tue, 19 Jan 2038 03:14:07 GMT`**

cookie 到了截止日期，浏览器会自动删除它。
...
一个可以替代 `expires` 的字段，具体说明 cookie 的过期时间距离当前时间的秒数。
...
因此，cookies 是基于域名的，它们不是通过协议来区分的。

有了这个选项，如果一个 cookie 通过 `https://site.com` 设置，然后它不会在相同域名的 HTTP 环境下出现，例如 `http://site.com`。所以，如果一个 cookie 存有敏感内容，不应该在不安全的 HTTP 环境下发送，此时这个选项就是派上用场了。
...
想象一下，你登录了 `bank.com` 网站。此时：你有了一个身份验证的 cookie。你的浏览器会随着每次请求把它发送到 `bank.com` ，因此，`bank.com` 承认你的身份和你做出的所有敏感经济操作。

现在，当你在另外一个窗口浏览网页时，你偶然到了另外一个网站 `evil.com`，它自动提交了一个表单 `<form action="https://bank.com/pay">` 到 `bank.com`，提交的表单字段能够开始一笔到黑客账户的交易。

表单直接从 `evil.com` 提交到银行的网站，并且你的 cookie 也一起发送，仅仅因为 cookie 会在你每次访问 `bank.com` 时都会发送。所以银行承认你和你做出的付款行为。

![](cookie-xsrf.png)

这就被称为一个跨站点请求伪造（或XSRF）攻击。

当然，真正的银行会防止出现这种情况。所有 `bank.com` 生成的表单都有一个特殊的字段，所谓的 "xsrf 保护令牌”，然后邪恶页面既不能生成，也不能从远程页面获取（它可以在那里提交，但是不能获取数据）。

但这需要时间来实现：我们需要保证每个表单都有这个 token 字段，而且必须检查所有的请求。

### 填写 cookie samesite 选项
...
换句话说，无论用户是跟踪邮件链接或从 `evil.com` 提交表单，或者来自其他域名下的任何操作，cookie 都不会被发送。

如果身份验证的 cookies 存在 `samesite` 选项，XSRF 攻击是没有机会成功的，因为 `evil.com` 发起的提交没有 cookies，所以 `bank.com` 不能识别用户，并且不会发起交易。

保护非常有效。只有来自 `bank.com` 的操作才会发送 `samesite` cookie。

虽然，这样有一点点不方便。

当一个用户跟随合法链接来到 `bank.com`，例如他们自己的笔记，令他们感到惊讶的是 `bank.com` 不能识别他们的身份。实际上，在这种情况下 `samesite=strict` cookies 不会发送。

我们可以使用两个 cookies 来解决这个问题：一个 cookie 用来大致识别，仅仅用来说 "Hello, John"，另外一个带有 `samesite=strict` 的 cookie 用来验证数据改变的操作。然后来自外部网站的用户会看到欢迎页面，但必须在银行的网站上发起付款。
...
1. HTTP 方法是安全的（例如 GET 方法，不是 POST）。
    ...
    所有安全的 HTTP 方法列表可以看 [RFC7231 规范](https://tools.ietf.org/html/rfc7231)。基本上，这些都是只能用来读取数据的方法，不能写入数据。他们不能执行任何更改数据的操作。以下链接都是 GET 安全的方法。

2. 操作执行顶级导航（在浏览器地址栏中改变 URL）。

    这通常是正确的，但是如果导航是在一个 `<iframe>` 中执行，那么它不是顶级的。同时，AJAX 请求不执行任何导航，因为它们不适合。
...
这被用作预防措施，以防止黑客将自己的 JavaScript 代码注入页面并等待用户访问页面时发起的攻击。这应该是不可能发生的，黑客应该不可能将他们的代码注入我们的网站，但是网站有可能存在漏洞让黑客利用来实现这样的操作。
...
使用给定的 `value` 设置 `name` cookie，默认情况是 `path=/`（可以修改或添加其他默认值）：
...
当 cookie 在用户正在访问的域名外设置，则被称为第三方。

例如：

1. `site.com` 网站的一个页面加载了另外一个网站的 banner：`<img src="https://ads.com/banner.png">`。
2. 和 banner 一起，`ads.com` 的远程服务器可能设置 `Set-Cookie` 头部比如 `id=1234`。此类 cookie 源自 `ads.com` 域名，并且只在 `ads.com` 下是可见的：
...
4. 更为重要的是，当用户从 `site.com` 网站移动到另外一个带有 banner 的 `other.com` 上时，然后 `ads.com` 获取它所属的 cookie，然后识别用户身份并跟踪他在网站之间跳转：

    ![](cookie-third-party-3.png)

由于它的性质，第三方 cookies 传统上用于跟踪和广告服务。它们绑定在原始域名上，然后 `ads.com` 可以跟踪到不同站点下的相同用户，如果他们访问的话。
...
欧洲有一项 GDPR 的立法，它对网站实施一套尊重用户隐私的规则。其中一条规则是要求用户明确许可了才可以跟踪 cookies。

请注意，这只是关于跟踪/识别 cookies。

所以，如果我们设置了一个保存了一些信息的 cookie，但是既不跟踪也不识别用户，那么我们可以自由的设置它。

但是如果我们要设置带有身份验证的 cookie 或跟踪 id，那我们必须得到用户允许。

网站在遵循 GDPR 下通常有两种做法，你应该已经在网站中看到过它们了：

1. 如果一个网站想要为已经经过身份验证的用户设置跟踪 cookies。

    为此，注册表格必须要有一个选择框，例如 "接受隐私政策"，用户必须勾选了，然后网站才可以自由设置身份验证 cookies。

2. 如果一个网站想要为每个人设置跟踪 cookies。

    为了合法实现，网站为新用户显示一个模态框，然后要求他们同意设置 cookies。然后网站才能设置 cookie 并且让用户看到网站内容。虽然这对新用户来说可能是令人不安的。没有人喜欢看到 "必须点击" 的模态框而不是内容。但是 GDPR 要求得到用户明确的同意。
...
- name/value 必须编码。
- 一个 cookie 最大 4kb，每个网站最多 20 几个（取决于浏览器）。

cookie 选项：
- `path=/`，默认为当前路径，使 cookie 仅在该路径下可见。
- `domain=site.com`，默认情况下 cookie 仅在当前域名下可见，如果明确设置了域名，可以让 cookie 在子域名下也可见。
